...
class ApiUnits():
    ...
    async def distributioncenters_get(
        self,
        business_id: str,
        country_id: str,
        ...
        Возвращает список ПРЦ (производственно-распределительный центр) с информацией о них.

        Документация: https://docs.dodois.io/docs/dodo-is/1bd039683b54a-zavedeniya-informacziya-o-pr-cz
        URL: https://api.dodois.io/dodopizza/ru/units/distributioncenters
        ...
        Аргументы:
            - business_id [str]: идентификатор бизнеса
                                (для Пиццы - 63d4829611ea45c8ae71394860a2481c,
                                 для Дринкит — c0b18e725258427a8bffea4f73957b0e)
        ...
        self.__distributioncenters_get_validate_scopes(user_scopes=user_data['scopes'])
        http_data: dict[str, Any] = self.__distributioncenters_get_http_params(
        ...
        while 1:
            ...
            return_data.extend(response.data["distributionCenters"])
            if response.data['isEndOfListReached'] or not take_all:
                break
            else:
                http_data['query_params']['skip'] += http_data['query_params']['take']
        return self.__distributioncenters_get_process_data(data=return_data)

    def __distributioncenters_get_process_data(
        self,
    ...
    def __distributioncenters_get_http_params(
        self,
        access_token: str,
        business_id: str,
        country_id: str,
        ...
        """
        Возвращает параметры HTTP запроса для distributioncenters_get.
        """
        BusinessIds.validate(value=business_id)
        CountryIds.validate(value=country_id)
        UnitStates.validate(value=unit_states)
        return {
            'method': HttpMethods.GET,
            'url': f'{self.__base_url}/distributioncenters',
            'query_params': {
                k: v
                for k, v
                in {
    ...
    def __distributioncenters_get_validate_scopes(
        self,
        user_scopes: Iterable[str],
    ) -> None:
        """
        Проверяет наличие обязательных scopes для метода distributioncenters_get.
        """
    ...
    def __shifts_get_http_params(
        ...
        return {
            ...
            'query_params': {
                ...
                for k, v
                in {
                    'from': convert_datetime_to_str(clock_in_from) if clock_in_from else None,
                    'to': convert_datetime_to_str(clock_in_to) if clock_in_to else None,
    ...
    async def stores_get(
        ...
        while 1:
            ...
            return_data.extend(response.data["stores"])
            if response.data['isEndOfListReached'] or not take_all:
                break
            else:
                http_data['query_params']['skip'] += http_data['query_params']['take']
